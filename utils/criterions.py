...
def my_softmax_dice_loss(output, target,eps=1e-5): #
    # output : [bsize,c,H,W,D]
    # target : [bsize,H,W,D]
    output=output[:,1,...]
    target=(target == 1).float()
    loss1 = dice(output,target)

    logging.info('1:{:.4f} '.format(1-loss1.data))
...
def GeneralizedDiceLoss(output,target,eps=1e-5,weight_type='square'): # Generalized dice loss
    ...
    # target = target.float()
    #print("target.dim() ",target.dim(),"output.size()[1]",output.size()[1])
    if target.dim() == 4:
        target[target == 4] = 3 # label [4] -> [3]
        #target = expand_target(target, n_class=output.size()[1]) # [N,H,W] -> [N,4,H,W]
    #print("target.shape:",target.shape,"target.size: ",target.size)
    output = flatten(output)[:,...] # transpose [N, 4,H,W] -> [4, N,H,W] -> [3, N*H*W] voxels
    target = flatten(target)[:,...] # [class, N*H*W]
    #print("target.shape:",target.shape,"target.size: ",target.size)
    ...
    #print("intersect.shape:",intersect.shape,"denominator.shape",denominator.shape)
