...
for mask_path in tqdm(mask_paths):
    ...
    for i in range(mask.shape[0]):
        mask_slice = mask[i]
        labeled, num_features = label(mask_slice > 0)
        ...
        for region_idx in range(1, num_features + 1):
            ...
            all_sizes_all.append(square_size)  # 保留所有尺寸，包括<20

# 全尺寸分布
size_counter_all = Counter(all_sizes_all)
sizes_all = np.array(sorted(size_counter_all.keys()))
counts_all = np.array([size_counter_all[s] for s in sizes_all])

# 寻找波峰
peaks_all, _ = find_peaks(counts_all, distance=5)
peak_sizes_all = sizes_all[peaks_all]
peak_counts_all = counts_all[peaks_all]

# 取前10大峰
top10_indices_all = np.argsort(peak_counts_all)[-10:][::-1]
top10_peaks_all = peak_sizes_all[top10_indices_all]
top10_counts_all = peak_counts_all[top10_indices_all]

# 绘制折线图
plt.figure(figsize=(12, 6))
plt.plot(sizes_all, counts_all, marker='o', linestyle='-', color='purple', label='All Lesion Sizes')
plt.scatter(top10_peaks_all, top10_counts_all, color='orange', s=50, label='Peaks')
for peak in top10_peaks_all:
    plt.axvline(x=peak, color='g', linestyle='--', linewidth=1)
plt.xlabel('Lesion Square Size (pixels)')
plt.ylabel('Count')
plt.title('Lesion Square Size Distribution (All Sizes Included)')
plt.grid(True, linestyle='--', alpha=0.7)
plt.legend()
plt.tight_layout()
plt.savefig('model/multi_mamba/counts_all.png', dpi=300)
plt.close()

print("\nTop 10 Peaks for All Sizes (pixels), Counts, and Size/Image Ratio:")
img_size = 160  # 图像边长
for s, c in zip(top10_peaks_all, top10_counts_all):
    ratio = s / img_size
    print(f"Size: {s}, Count: {c}, Ratio: {ratio:.3f}")